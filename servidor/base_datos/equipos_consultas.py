    ...
    def obtener_equipos_computo(self):
        if self.conexion.is_connected():
            try:
                ...
                cursor.execute(f"SELECT e.area_equipo, e.nombre_equipo, e.numero_serie_equipo, CONCAT(p.nombre_propietario, ' ', p.apellido_propietario) as propietario_equipo,CASE WHEN e.rol = 1 THEN 'administrador'WHEN e.rol = 0 THEN'cliente' END as rol FROM equipos as e JOIN propietarios as p ON e.propietario_equipo = p.id_propietarios ORDER BY e.rol DESC")
